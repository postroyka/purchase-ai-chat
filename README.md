...
ИИ-сервис, который превращает счёта и прайс-листы поставщиков (PDF, фото/скан, XLSX/XLS, DOCX) в **структурированные документы закупки** и проводит их в учётную систему. Доступ — через **REST API** (загрузка/статус/метрики) и **слой интеграций на MCP**.

**Зачем:** менеджер не вбивает счёт руками — загрузил файл, система распознала поставщика, договор, позиции, цены и завела документ закупки. Экономия времени на каждой закупке (оценка — на дашборде `/metrics`).

**Как это работает:** REST `POST /upload` → backend извлекает текст (PDF/фото — OCR, office — python-хелпер) → ИИ-агент (Claude Code) извлекает из текста структуру → проводит документ через **MCP-инструменты**. **Учётная система подключается ТОЛЬКО через MCP** — ядро разбора о ней ничего не знает: сегодня бэкенд интеграции — **Bitrix24** (сделка в воронке «Закупки», BYN, НДС 20%); на очереди — **1С: Управление торговлей** (тот же MCP-слой, меняется только транспорт — [docs/1C_UT_INTEGRATION.md](docs/1C_UT_INTEGRATION.md)).

**Программный доступ (REST API, Bearer/сессия):** `POST /upload` (файлы → задание), `GET /job/:id/status` (статус и результат разбора), `GET /metrics/data` (метрики), `POST /feedback` (обратная связь). UI — тонкий клиент поверх этих же эндпоинтов.
...
| `PUBLIC_PAGE_RESPONSIBLE_USER_ID` | app | — | Ответственный за сделку по умолчанию (**фолбэк**). Внутри Битрикс24 сделка назначается на загрузившего сотрудника (его id из профиля b24jssdk); этот дефолт срабатывает вне фрейма (публичная страница) или если id недоступен. Часто = владелец вебхука |
...
| `RATE_LIMIT_MAX` | app | — | Лимит запросов `/upload` на токен в окне. По умолчанию `0` = **выкл.** (внутренний инструмент); `>0` включает анти-DoS |
...
**Ролевые инструкции (краткие, по аудиториям):**
- [docs/GUIDE_USER.md](docs/GUIDE_USER.md) — **сотруднику**: где открыть, что вставить, как получить результат, что делать если не по плану
- [docs/GUIDE_ADMIN.md](docs/GUIDE_ADMIN.md) — **сисадмину**: профилактика сервера (обновления, логи, место, бэкапы) — регламент день/неделя/месяц
- [docs/GUIDE_OWNER.md](docs/GUIDE_OWNER.md) — **владельцу репозитория**: токены (что есть, где, ротация) и работа с Git (PR → CI → прод, релизы/откат)
- [docs/1C_TZ.md](docs/1C_TZ.md) — **1С-программисту**: краткое ТЗ интеграции (полный контракт — `1C_UT_INTEGRATION.md`)

**Планы и эксплуатация:**
- [docs/USER_GUIDE.md](docs/USER_GUIDE.md) — **руководство** (расширенное): сотруднику (как загрузить счёт и прочитать результат), владельцу/админу (чек-лист запуска + как растить точность по `/metrics`)
