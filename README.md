...
**Обновить upstream MCP** (подробно, с закреплённым SHA снимка — [`mcp/VENDOR.md`](mcp/VENDOR.md)):
```bash
# ПО ТЕГУ, не main (main движется без ревью). Актуальную версию см. в mcp/VENDOR.md.
git subtree pull --prefix=mcp https://github.com/bitrix24/templates-mcp <vX.Y.Z> --squash
```
После обновления: сверить дельту с записанным SHA, обновить `mcp/VENDOR.md`, убедиться,
что overlay (`mcp-overlay/`) не конфликтует и проходит `pnpm typecheck` (см. `mcp/VENDOR.md`).
