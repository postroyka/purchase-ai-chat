...
2. Открыты порты 80 и 443, настроен firewall. Исходящий HTTPS к `api.nbrb.by` не заблокирован (курс USD→BYN для метрик; при блокировке — тихий фолбэк на `USD_BYN_RATE`).
