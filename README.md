...
✅ **Статус:** сквозной путь реализован полностью — backend, агент (`backend/agent-runner.js`, спавн Claude Code CLI) и 4 PST-инструмента (`b24_pst_crm_*`: find-supplier / find-contract / find-product / create-deal) создают сделку в Б24 через внешний b24-controller. Инструменты живут в `mcp-overlay/` (см. ниже).
...
> Для многих задач (UI, загрузка, jobs) backend MCP локально не нужен — его можно не запускать; для сквозной проверки инструментов поднимите MCP (:3002).
