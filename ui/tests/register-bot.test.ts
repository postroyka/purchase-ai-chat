...
describe('registerInvoiceBot (#217 — LEGACY imbot.register; портал старый)', () => {
  // Аватар — fire-and-forget fetch('/botavatar.png'); по умолчанию глушим (ok:false → без imbot.update),
  // чтобы тесты регистрации не били в сеть (happy-dom иначе → ECONNREFUSED) и не считали лишний вызов.
  beforeEach(() => {
  ...
  it('аватар — fire-and-forget: imbot.update не блокирует регистрацию, его провал её не валит', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => ({ ok: true, arrayBuffer: async () => new Uint8Array([1, 2, 3, 4]).buffer })))
    const { frame, make } = frameStub(async (opts) => {
      if (opts.method === 'imbot.register') return ok(456)
      if (opts.method === 'imbot.command.register') return ok(99)
      if (opts.method === 'imbot.update') return fail(['PHOTO_ERROR']) // даже провал фото не виден caller'у
      throw new Error(`unexpected ${opts.method}`)
    })
    const r = await registerInvoiceBot(frame, 'https://app/x')
    expect(r).toEqual({ botId: 456, commandId: 99 }) // вернулись независимо от аватара
    // imbot.update долетает в фоне — дожидаемся и проверяем форму запроса
    await vi.waitFor(() => expect(make.mock.calls.some(c => c[0].method === 'imbot.update')).toBe(true))
    const upd = make.mock.calls.find(c => c[0].method === 'imbot.update')![0]
    expect(upd.params).toMatchObject({ BOT_ID: 456 })
    expect((upd.params.FIELDS as { PROPERTIES: { PERSONAL_PHOTO: string } }).PROPERTIES.PERSONAL_PHOTO).toBeTruthy()
  })
})
