...
- **НДС-модель позиций (#326, утверждено заказчиком по итогам прод-проверки).** Портал настроен
  «цены содержат НДС», поэтому:
  - цена за единицу пишется в **`PRICE_BRUTTO`** (1-в-1 с бумажным счётом; `PRICE` давал дрейф
    4.35→4.36, `PRICE_NETTO` показывал брутто ×1.2 — отвергнуты по тестам Т1/Т2);
  - `TAX_RATE = 20`, `TAX_INCLUDED = Y`, единица = шт;
  - «Итого»/«НДС» строки прибиваются **явно**: `SUM = цена × кол-во`, `TAX_SUM = SUM × 20/120`
    (НДС, включённый в Итого, = `SUM/6`).
  - ⚠️ **Зависимость от кастомной правки ядра заказчика.** Расчёт держится на доработке
    `CCrmProductRow::SaveRows`, которая **уважает переданные `SUM`/`TAX_SUM`** (штатный Bitrix их
    пересчитывает сам). Это правка **в ядре** (`bitrix/modules/crm/...`) → **перезатирается при
    обновлении Bitrix**; без неё колонки «НДС»/«Итого» опустеют. Правку нужно **сохранять**
    (вынести в обработчик события или ре-патчить после апдейтов). Если значения «Итого»/«НДС»
    в сделках вдруг пропали — первым делом проверять эту правку.
