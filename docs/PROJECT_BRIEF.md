...
- **Неделя 3** — почти закрыта. End-to-end работает в B24 (тест-портал: установка → загрузка → сделка → «Открыть сделку»), выкачено в прод (v0.1.1–v0.1.3).
  ...
  - 🆕 **Обратная связь (issue #182):** канал «сотрудник» (оценка 👍/👎/💡 + комментарий на странице результата) **и** канал «агент» (агент возвращает `warnings[]`/`feedback[]` → счётчики на `/metrics` + GitHub issue с дедупом). `backend/feedback.js`, `backend/agent-feedback.js`, `prompts/main.md`, `docs/FEEDBACK.md`. Канал «MCP» остаётся открытым в #182.
...
**Открытые задачи (issues):** #182 (фидбек: каналы «сотрудник» ✅ и «агент» ✅ сделаны; «MCP» — ещё открыт) · #176 (автозавод кастом-полей при установке) · #177 (рус. форма входа, нужна только вне B24) · #174/#175 (харднинг).
