...
#
#  MCP-инструменты b24_pst_crm_* реализованы (PR #71/#86): успешный прогон
#  создаёт реальную сделку в Bitrix24 (job 'done' + dealId в результате).
#  Ошибка на этапе MCP = реальная проблема (MCP/b24-controller/токен), не заглушка.
#
...
if [ "$FILE_STATUS" = "done" ]; then
  ...
  echo "⚠ Агент ЗАПУСТИЛСЯ, авторизовался, прочитал файл и ДОШЁЛ до MCP, но"
  echo "   инструмент/соединение дали ошибку. Инструменты b24_pst_crm_* УЖЕ реализованы"
  echo "   (PR #71/#86) — значит это РЕАЛЬНАЯ проблема (MCP недоступен / b24-controller /"
  echo "   токен/контракт), а НЕ заглушка. Разберите ошибку выше. Успех = job 'done' + dealId."
