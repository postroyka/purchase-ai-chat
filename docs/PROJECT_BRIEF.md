...
- **Неделя 3** — почти закрыта. End-to-end работает в B24 (тест-портал: установка → загрузка → сделка → «Открыть сделку»), выкачено в прод (v0.1.1–v0.1.3).
  ...
  - 🆕 **Обратная связь (issue #182) — все три канала готовы:** «сотрудник» (оценка 👍/👎/💡 + комментарий) · «агент» (`warnings[]`/`feedback[]` → счётчики на `/metrics` + GitHub issue с дедупом) · «MCP» (секция **«Проблемы матчинга»** на `/metrics`: рейтинг причин + поставщики без матча по УНП, `metrics.recordMatching`). `backend/feedback.js`, `backend/agent-feedback.js`, `prompts/main.md`, `docs/FEEDBACK.md`.
...
**Открытые задачи (issues):** #182 ✅ (все три канала фидбэка) · #190 (харднинг фидбэка: страховка приватности + единый контракт ✅; durable‑outbox — открыт, для v1 не нужен) · #176 (автозавод кастом-полей при установке) · #177 (рус. форма входа, нужна только вне B24) · #174/#175 (харднинг).
