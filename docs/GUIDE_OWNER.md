...
> На время паузы отбрасываются и входящие вебхуки Bitrix24 (сообщения бота, `ONAPPINSTALL`/`ONAPPUNINSTALL`) — это намеренно (пауза = полная остановка). Если в окне паузы приложение переустановят на портале — после снятия паузы проверьте, что оно всё ещё установлено/авторизовано.
