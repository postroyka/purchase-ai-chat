...
- **Bearer token leak (HTTP modes).** With `NUXT_BITRIX24_OAUTH_ENABLED=false` (the default), `NUXT_MCP_AUTH_TOKEN` is the only thing between a public `/mcp` and tool execution against your portal. It's compared with `crypto.timingSafeEqual`. Rotation procedure below. **Token hardening (issue #105):** a token shorter than 32 chars is rejected as "not configured" (`/mcp` → 503) so a typo/truncated value can't silently weaken auth (intended: `openssl rand -hex 32` = 64 chars). **Brute-force defence (issue #105):** failed webhook-token attempts are rate-limited per source IP — 10 failures / 60 s → `429` (errorCode `RATE-LIMITED`, `Retry-After`), checked *before* the timing-safe compare; only wrong-token attempts count, a successful auth clears the IP's counter, and the bucket map is LRU-bounded (process-local, same posture as `oauth-rate-limit.ts`). `/mcp` is internal-only (`procure-net`) in the reference deploy, so this is defence-in-depth over the token. Denials log `mcp.auth.deny.rate-limited` (WARN).
