	...
	extends Engine\Controller
{
	 ...
	 *
	 * Возвращает КАРТУ `vendorCode → результат` для КАЖДОГО запрошенного артикула:
	 * `{ id, name, vendorCode[, multi] }` если найден, иначе `{ id: null }`. Пустые
	 * элементы списка отбрасываются; дубликаты схлопываются (ключ карты — уникальный
	 * очищенный артикул).
	 *
	 * @param string[] $vendorCodes Артикулы поставщика из документа
	 * @return array<string,array>|null Карта артикул→результат | null при guard-ошибке
	 */
	public function findByVendorCodesAction(array $vendorCodes): ?array
	{
		...
		// Карта по КАЖДОМУ запрошенному артикулу (не найденные → id:null), как у одиночного метода.
		$out = [];
		foreach($codes as $vc)
		{
			$out[$vc] = $byArticle[$vc] ?? ['id' => null];
		}
