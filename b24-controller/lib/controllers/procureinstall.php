 ...
 *
 * Подключён к установке (#176): страница установки UI (ui/app/utils/ensure-schema.ts из
 * useInstall) вызывает этот метод через scope `crm` ДО installFinish и показывает итог.
 * Идемпотентность позволяет звать его на каждой установке; ручной вызов остаётся как фолбэк.
 *
