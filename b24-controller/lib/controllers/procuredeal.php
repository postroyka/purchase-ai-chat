	...
	extends Engine\Controller
{
	...
	): ?array
	{
		...
		$dealFields = [
			'TITLE'          => 'Импорт прайса от '.$supplierName,
		...
		{
			// Серверная страховка от «не найдено в каталоге»: если у позиции ЕСТЬ артикул
			// поставщика (vendorCode), но товар по нему не сопоставлен (productId пуст/0) —
			// в сделку её НЕ кладём (см. prompts/main.md, Шаг 4). Это дублирует правило
			// промпта на уровне контроллера, т.к. модель не всегда исключает такие позиции
			// сама. Позиции БЕЗ артикула (свободная строка) остаются как есть.
			if(!empty($item['vendorCode']) && empty($item['productId']))
			{
				continue;
			}
