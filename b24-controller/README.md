...
> В сделку кладутся **только** позиции с сопоставленным товаром (`productId` непустой). Позиция
> без `productId` — артикул не найден в каталоге **или** артикула нет вовсе — в сделку **не**
> кладётся (#258: свободные строки `PRODUCT_ID=0` больше не создаём; см. Шаг 4 промпта). Заголовок
> сделки — «Импорт прайса от &lt;название компании-поставщика&gt;»; если поставщик по УНП не найден —
> «Импорт прайса — поставщик не найден (УНП …)», сделка **без** `COMPANY_ID` + warning `supplier_not_found`.
...
В процесс установки приложения метод **подключён** (#176): страница установки `/install`
вызывает его (scope `crm`) до `installFinish` и показывает итог; повторный вызов безопасен
(см. [`docs/BITRIX24_APP_SETUP.md`](../docs/BITRIX24_APP_SETUP.md)). Деплоится как обычный
...
## Деплой (ручной и автоматический по тегу `v*`)

На **каждый push в `main`** деплой НЕ запускается — авто-триггер только релизный тег `v*`
(синхронно с прод-образами; см. «Автоматический деплой» ниже). Ручная выкладка по команде:
...
выкладывает контроллеры на **одну** коробку (задаётся repository-секретами) — **синхронно
с прод-образами** ([`deploy.yml`](../.github/workflows/deploy.yml)):

- **тег `v*`** (релиз) → после проверки, что CI на этом коммите зелёный, контроллеры
  выкладываются на коробку (бэкап + `php -l` + read-only health + **авто-rollback при
  провале** — в скрипте). Это **тот же** триггер, что двигает образы на `:latest`,
  поэтому PHP-сторона и Docker-образы релизятся **вместе**.
- **`workflow_dispatch`** (Actions → Deploy b24-controller → Run) → ручной запуск;
  можно включить **dry_run** (симуляция, без записи на сервер). Тоже под гейтом
  зелёного CI на коммите.

> Авто-деплой на **каждый push в `main`** убран намеренно: PHP катился сразу, а образы —
> только по тегу `v*`, и стороны рассинхронивались. Теперь обе едут по релизному тегу.
> Локальный `make deploy-b24 APPLY=1` остаётся рабочим как ручной путь **без** CI-гейта
> (отладка, ad-hoc, выкладка между релизами).
...
> **Откат релиза целиком.** Слои откатываются НЕЗАВИСИМО: образы — вернуть
> `APP_IMAGE_TAG`/`MCP_IMAGE_TAG` на предыдущий `sha-…` в `.env.prod` (корневой README,
> раздел про откат образа) + `make prod-up`; PHP — restore из бэкапа на коробке (раздел
> «Откат» выше). При контрактном изменении откатывайте ОБА слоя, PHP-сторону — в порядке,
> обратном выкладке (см. ниже).
...
- бизнес-правила позиций сделки (`TAX_RATE=20`, `TAX_INCLUDED=N` — цена нетто, НДС сверху (#325), «шт»);
