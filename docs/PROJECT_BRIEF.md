...
| `GITHUB_FEEDBACK_TOKEN` | вкл. канал обратной связи (issue #182): отзыв сотрудника → GitHub issue. Нет токена → виджет скрыт, `POST /feedback` → 503. fine-grained PAT, право Issues:write. См. `docs/FEEDBACK.md`. |
...
- **Неделя 3** — почти закрыта. End-to-end работает в B24 (тест-портал: установка → загрузка → сделка → «Открыть сделку»), выкачено в прод (v0.1.1–v0.1.3).
  ...
  - 🆕 **Обратная связь, канал «сотрудник» (issue #182):** на странице результата — оценка 👍/👎/💡 + комментарий → бэкенд заводит GitHub issue (`backend/feedback.js`, `POST /feedback`, `docs/FEEDBACK.md`). Каналы «MCP» и «агент» (агрегация `warnings[]` на `/metrics`) остаются открытыми в #182.
...
**Открытые задачи (issues):** #182 (фидбек: канал «сотрудник» ✅ сделан в этом PR; «MCP» и «агент» — ещё открыты) · #176 (автозавод кастом-полей при установке) · #177 (рус. форма входа, нужна только вне B24) · #174/#175 (харднинг).
