 ...
 *   Каждая позиция: цена в PRICE_BRUTTO (1-в-1), TAX_RATE=20, TAX_INCLUDED=Y, единица=шт; «Итого»/«НДС»
 *                   прибиваются через SUM/TAX_SUM (НДС включён, #326 — см. блок ниже и IMPLEMENTATION_NOTES)
	...
	extends Engine\Controller
{
	...
	): ?array
	{
		...
		// --- 2) Товарные позиции (цена в PRICE_BRUTTO 1-в-1, НДС включён через SUM/TAX_SUM, #326) ---
		...
		foreach($items as $item)
		{
			...
			// НДС-модель (#326, утверждено заказчиком по итогам прод-проверки). Портал «цены содержат НДС»,
			// поэтому строковые итоги считаем как «налог включён»: Итого = цена×кол-во; НДС = Итого×20/120 (=Итого/6).
			// Эти значения прибиваем явно через SUM/TAX_SUM (кастомная правка ядра CCrmProductRow::SaveRows
			...
			$productRows[] = [
				...
				// Цена кладётся в PRICE_BRUTTO → на портале «цены с НДС» показывается как «Цена» 1:1
				// (PRICE давал дрейф 4.35→4.36; PRICE_NETTO показывал брутто ×1.2 — см. IMPLEMENTATION_NOTES).
				'PRICE_BRUTTO' => $price,
				'QUANTITY'     => $quantity,
				'TAX_RATE'     => 20,
				'TAX_INCLUDED' => 'Y',
				// SUM — «Итого» строки; TAX_SUM — «НДС» (включённый). Прибиваем точные значения.
				'SUM'          => $sum,
				'TAX_SUM'      => $taxSum,
