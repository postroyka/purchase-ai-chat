...
- **Недели 1–2** — ✅ закрыты.
- **Неделя 3** — почти закрыта. End-to-end работает в B24 (тест-портал: установка → загрузка → сделка → «Открыть сделку»), выкачено в прод (v0.1.1–v0.1.3).
  ...
  - ⚠️ **Отклонение от ТЗ:** публичная страница — вместо HTTP Basic встроенная **форма входа** (сессия `pai_sess`), флаг `PUBLIC_PAGE_ENABLED` убран. Причина: Basic несовместим с iframe Битрикс24 (из-за него приложение не грузилось в портале). Согласовать обновление ТЗ.
...
- **Неделя 4** — следующая: повторный прогон (после правки артикулов клиентом), демо, передача; раскат на **боевой портал** `b24.postroyka.by`.
