...
`Последняя ревизия: 2026-06-23` · Статус: **❄️ ЗАМОРОЖЕН (#241)**.

> **Бот заморожен.** Портал-QA подтвердил: бот **работает** на старом портале (регистрация, события,
> авто-захват токена через `app.info`, ответ). Но чат-сценарий деприоритизирован, регистрацию **убрали из
> установки** (`ui/app/composables/useInstall.ts` — закомментирована); код и бэкенд-обработчики **оставлены**
> для будущих версий Битрикс24. Открытый хвост (форма скачивания файла) и порядок возврата — в [#243] и
> ниже. Документ ведём как **дизайн + лог портал-QA**; разделы ниже актуальны на момент заморозки.

> ⚠️ **ТЕКУЩАЯ РЕАЛИЗАЦИЯ — LEGACY-API (#241).** Портал заказчика (`tstb24.postroyka.by`) **старый**:
> `imbot.v2.Bot.register` → **404**, API чат-ботов 2.0 (`imbot.v2.*`, события `ONIMBOTV2*`) недоступен.
> Поэтому в коде сейчас **legacy**: регистрация `imbot.register`, события `ONIMBOTMESSAGEADD` /
> `ONIMBOTJOINCHAT` / `ONIMBOTDELETE` / `ONIMCOMMANDADD`, ответ `imbot.message.add`, скачивание файла —
> по ссылке из самого события (`data.PARAMS.FILES[].urlDownload`). **v2-код не удалён — закомментирован**
> блоками «=== v2 (вернуть при тираже) ===» в `register-bot.ts` / `b24-bot.js` / `b24-bot-api.js`; на
> новом проде вернёмся к v2. Разделы ниже описывают v2-дизайн — читать с поправкой на этот баннер.
>
> 🔎 **Открытый момент:** форма `data.PARAMS.FILES` на старом портале сверяется по дебаг-логу
> (`B24_BOT_DEBUG`, по умолчанию вкл; логирует только `MESSAGE`/`FILES` + `bot{id,endpoint,hasToken}`, без
> токенов) — снять на первом прогоне и при необходимости поправить `resolveDownloadUrl` в `b24-bot-api.js`.
>
> ✅ **Живой портал-прогон (#241) выявил и исправлено:** (1) события бота **доходят** (EVENT_HANDLER ок),
> но ответ слался JSON-телом → старый портал давал `NO_AUTH_FOUND`: теперь `imbot.message.add` шлётся как
> `app.info` — `auth` в query + параметры form-urlencoded (`toFormBody`). (2) access_token для ответа берётся
> и из top-level `auth` события (не только из `data.BOT[id]`). (3) `ONAPPINSTALL` на callback **не приходит**
> (старый портал) — поэтому `application_token` захватывается из **ПЕРВОГО подлинного события бота**
> (`app.info` + allowlist, как при установке), без ручного env-токена. (4) Аватар — `imbot.update`
> `PERSONAL_PHOTO` (best-effort, ассет `/botavatar.png`), ставится в фоне при (пере)установке.
...
- ~~**Установка получает серверный шаг:** клиентский POST токена на backend-роут~~ — **заменено
  ...
  §10). Клиенту остаётся только `imbot.v2.Bot.register` + `imbot.command.register`.
...
2. **Захват токена (#217, предпочтительно):** в карточке приложения указать **«Ссылка-callback для
   события установки»** = `https://<тестовый-домен>/b24/app/event` и **переустановить** — токен
...
3. Регистрация бота происходит **автоматически** при установке (`register-bot.ts` → `useInstall`):
   **LEGACY** `imbot.register` (`EVENT_HANDLER = https://<тестовый-домен>/b24/bot/event`) + команда
   ...
   старом портале; вернём при тираже, #243.)_
