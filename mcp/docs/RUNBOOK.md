...
| Client reports **429** on `/mcp` (webhook mode) | Per-IP brute-force limit hit: ≥10 failed auth attempts / 60 s (issue #105). Only WRONG-token attempts count; a valid token clears the counter, so this means repeated bad tokens from that IP. | Grep `mcp.auth.deny.rate-limited` for the `ip`. A single source self-resolves in ~60 s. If a legitimate client trips it, fix its `Authorization: Bearer <NUXT_MCP_AUTH_TOKEN>` (typo/stale token) and wait for the window. A sustained flood from an unknown IP on the internal `/mcp` ⇒ investigate network exposure (it should be `procure-net`-only). Thresholds are in `server/middleware/mcp-auth.ts` (`MAX_FAILS_PER_WINDOW`, `FAIL_WINDOW_MS`). |
