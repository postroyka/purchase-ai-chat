...
// `make` отвечает по методу: ensureSchema → отчёт, остальное → id. (Чат-бот заморожен #241 — при
// установке больше не регистрируется, поэтому imbot.* сюда не приходит.)
...
describe('useInstall — подтверждение установки приложения Битрикс24', () => {
  ...
  it('install-режим: ensureSchema через actions.v2.call.make СТРОГО ДО installFinish (#176)', async () => {
    // Лог порядка: make и installFinish пишут в него — донастройка схемы обязана быть до finish,
    // иначе перезагрузка фрейма после installFinish оборвала бы REST-вызов. (Чат-бот заморожен #241 —
    // при установке больше НЕ регистрируется, поэтому imbot.* в последовательности нет.)
    const order: string[] = []
  ...
  it('ensureSchema провалился (нет scope crm) → schemaStatus=failed, установка не сорвана (#176)', async () => {
    ...
    expect(frame.installFinish).toHaveBeenCalledTimes(1)
    expect(state.value).toBe('done')
    expect(botWarning.value).toBe('') // бот заморожен (#241) — предупреждения про бота не возникает
  ...
  it('ensureSchema: REST ok, но result=null (сбой загрузки модулей в PHP) → schemaStatus=failed (#176)', async () => {
