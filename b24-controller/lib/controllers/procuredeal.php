 ...
 *   Каждая позиция: TAX_RATE=20, TAX_INCLUDED=Y (откат #326 на перепроверку), единица=шт, цена = priceExclVat
	...
	extends Engine\Controller
{
	...
	): ?array
	{
		...
		// --- 2) Товарные позиции (TAX_RATE=20, TAX_INCLUDED=Y — откат #326 на перепроверку НДС-модели) ---
		...
		foreach($items as $item)
		{
			...
			// ТЕСТ 3.1 (#326-re): НДС включён в цену (портал «цены с НДС»), поэтому считаем как в счёте,
			// что показал заказчик: Итого = цена×кол-во; НДС, включённый в Итого = Итого×20/120 (=Итого/6).
			// Эти значения прибиваем явно через SUM/TAX_SUM (кастомная правка ядра CCrmProductRow::SaveRows
			// у заказчика их уважает) — иначе колонки «НДС»/«Итого» остаются пустыми. round до копеек.
			...
			$productRows[] = [
				...
				// (Тест 1 PRICE→4.36 дрейф; Тест 2 PRICE_NETTO→5.22 брутто; Тест 3 PRICE_BRUTTO→цена ровно).
				'PRICE_BRUTTO' => $price,
				'QUANTITY'     => $quantity,
				'TAX_RATE'     => 20,
				'TAX_INCLUDED' => 'Y',
