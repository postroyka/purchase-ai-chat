...
- **Неделя 3** — почти закрыта. End-to-end работает в B24 (тест-портал: установка → загрузка → сделка → «Открыть сделку»), выкачено в прод (v0.1.1–v0.1.3).
  ...
  - 🔧 **Долги недели 3 (в работе):** версия билда (git sha + ссылка) в подвале UI; авто-чистка `uploads/` по `UPLOADS_RETENTION_DAYS`.
...
**Открытые задачи:** #182 (срочно — сбор фидбека от пользователя/MCP/агента) · #176 (автозавод кастом-полей при установке) · #177 (рус. форма входа, нужна только вне B24) · #174/#175 (харднинг) · #181 (rename деплой-воркфлоу).
