...
> **Что считать успехом (Week 1).** Инструменты MCP (`b24_pst_crm_*`) пока
> заглушки, поэтому реальная сделка не создаётся. «Уровень 1 PASS» = агент
> запустился, авторизовался, прочитал файл и **дошёл до MCP** (упёрся в
> заглушку/недоступность). Скрипт распознаёт это состояние и помечает как
> ожидаемое. Полный флоу разблокируется в Week 2.
